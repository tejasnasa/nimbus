...
describe("socket: chat", () => {
  ...
  describe("presence", () => {
    it("announces a join to the room and replays the roster to the joiner", async () => {
      ...
      const rosterUpdate = waitForEvent<string[]>(bobSocket, "presence:online_users");
      bobSocket.emit("workspace:join", wsId);

      await expect(announcement).resolves.toMatchObject({ userId: bob.id });
      await expect(rosterUpdate).resolves.toEqual(expect.arrayContaining([alice.id, bob.id]));
    });
    ...
    it("records presence in Redis", async () => {
      ...
      await expect(presenceService.getOnlineUsers(wsId)).resolves.toContain(alice.id);
    });
    ...
    it("announces a leave and clears presence", async () => {
      ...
      const departure = waitForEvent<{ userId: string }>(bobSocket, "presence:left");
      aliceSocket.emit("workspace:leave", wsId);

      await expect(departure).resolves.toMatchObject({ userId: alice.id });
      await expect(presenceService.getOnlineUsers(wsId)).resolves.not.toContain(alice.id);
    });
  ...
  describe("messages", () => {
    it("persists a message and broadcasts it to everyone else in the room", async () => {
      ...
      const received = waitForEvent<{ content: string; userId: string; name: string }>(
        bobSocket,
        "message:new",
      );
      aliceSocket.emit("message:send", { workspaceId: wsId, content: "hello bob" });
      ...
      const stored = await testPrisma.message.findFirst({ where: { workspaceId: wsId } });
    ...
    it("drops a message from a non-member rather than persisting it", async () => {
      ...
      socket.emit("message:send", { workspaceId: wsId, content: "i should not exist" });
      await new Promise((resolve) => setTimeout(resolve, 300));
    ...
    it("relays typing indicators to others but not back to the sender", async () => {
      ...
      const relayed = waitForEvent<{ userId: string }>(bobSocket, "typing:start");
